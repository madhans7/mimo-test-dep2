...
def process_job(doc_snapshot):
    ...
    # Read copies and colorMode from printOptions first (new schema), fallback to top-level (legacy)
    copies = print_options.get("copies") or doc.get("copies", 1)
    color_mode = print_options.get("colorMode") or doc.get("colorMode", "monochrome")
    ...
    duplex = print_options.get("doubleSided", "single")
